...
func (this *HttpDownloader) changeCharsetEncodingAuto(header http.Header, sor io.ReadCloser) string {
	//var err error
	...
	//     destReader = sor
	// }
	charset := this.getCharset(header)
	bodystr := this.changeCharsetEncoding(charset, sor)
...
func (this *HttpDownloader) changeCharsetEncodingAutoGzipSupport(header http.Header, sor io.ReadCloser) string {
	var err error
	...
	charset := this.getCharset(header)
	bodystr := this.changeCharsetEncoding(charset, gzipReader)
...
func (this *HttpDownloader) downloadFile(p *page.Page, req *request.Request) (*page.Page, string) {
	...
	if resp.Header.Get("Content-Encoding") == "gzip" {
		bodyStr = this.changeCharsetEncodingAutoGzipSupport(resp.Header, resp.Body)
	} else {
		bodyStr = this.changeCharsetEncodingAuto(resp.Header, resp.Body)
	}
