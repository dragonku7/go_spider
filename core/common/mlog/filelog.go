...
func LogInst2() *filelog {

	if flog == nil {
		InitFilelog2(true, "")
	}
	year, month, day := time.Now().Date()
	if strconv.Itoa(year)+"-"+strconv.Itoa(int(month))+"-"+strconv.Itoa(day) != flog.file_create_date {
		flog = nil
		InitFilelog2(true, "")
	}
	return flog
...
func InitFilelog2(isopen bool, fp string) {
	if !isopen {
		flog = &filelog{}
		flog.loginst = nil
		flog.isopen = isopen
...
func newFilelog(isopen bool, logpath string) *filelog {
	...
	pfilelog.file_create_date = strconv.Itoa(year) + "-" + strconv.Itoa(int(month)) + "-" + strconv.Itoa(day)
